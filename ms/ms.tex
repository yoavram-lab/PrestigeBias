...
A very common bias in the literature is \textit{success bias}. Success bias refers to the tendency for certain cultural traits or behaviors to spread and become more common in a population simply because they have been successful in the past. Success in that regard is defined by each individual trait, for example, a successful manifestation is a ping pong-player who won many games, tournaments, didn't lose much, etc..
\citet{prestige_cultural_learning} describe a definition for success, as well as skill and prestige bias, and their differences.
...
%\paragraph{Prestige.}
Prestige is defined as the ``widespread respect and admiration felt for someone or something on the basis of a perception of their achievements or quality'' (New Oxford American Dictionary).
Therefore, prestige does not directly signify success, although it may imply it--making it an indirect bias.
...
Importantly, influence may be easier and more accurate to estimate compared to success.

Influence bias is a context-based bias, rather than a content-based bias--it does not depend on the phenotype but rather on the number of copiers that already copied each role-model. It also differs from frequency biases such as conformity, which depend on the frequency of a trait in the population or in a sample of role-models rather than the social dynamics of copying.

Here, we define a stochastic model of cultural transmission with prestige bias that combines both success and influence biases, find analytic approximations for this model, and analyze its dynamics. We find approximations for the probability and time for fixation of a `successful' phenotype (i.e., that is favored by success bias). Comparing these approximations to Kimura's approximations for the fixation of a favorable allele \citep{kimura,kimura_average}, we demonstrate that success and influence bias play the role of natural selection and genetic drift, respectively. 
