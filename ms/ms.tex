...
Each generation, $N$ naive individuals, or copiers, choose an individual from the previous generation, or role-models, from which they will copy their trait. Similar to a Wright-Fisher model, {generations are non-overlapping, and}  the entire population is replaced in each generation.
...
{Boyd and Richerson} \citep[Ch.~8, p.~247-249]{evolutionBook} describe a blended transmission algorithm by defining $F$,  a weighted average of the traits of all role-models, {as}
...
We found two such approximations, summarized here and explained in detail below. In both we assume either $\alpha_j$ is a trait of role-model $j$ only ({different from} $\alpha_{ij}$ that included the relation with copier $i$), or a completely homogeneous $\alpha$.
...
{Frigyik et al.}\ \citep[section 2]{dirichlet} prove that the proportion of different colored balls in a \emph{P\'{o}lya urn model} converges to the Dirichlet distribution as the number of draws approaches infinity, based on the \emph{Martingale Convergence Theorem} \citep{martingaleBook}.
...
\begin{figure}[h]
  ...
  \textbf{(B)} The log-likelihood of the DM distribution for results of the experiments (red vertical line) is much higher {than} the log-likelihood of permutations of experiments (blue histogram).
