...
Source code is available on GitHub (\href{https://github.com/yoavram-lab/PrestigeBias}{https://github.com/yoavram-lab/PrestigeBias}) and deposited on Zenodo (doi:10.5281/zenodo.11424199).
...
Figure S1 shows some examples of model dynamics for various values of $\alpha$.
...
Although basic, Figure S2 shows a good fit of the GBD approximation.
...
We then compared this observed distribution with the expected theoretical DMD (Figure S3A).
...
In addition, we tested the likelihood of the observed data to be drawn from the DMD, against a shuffle of the parameters vector of the DMD itself (Figure S3B). We see that the negative log likelihood of the observed data is much higher than any other shuffled version of the parameters vector, strongly supporting our approximation.
...
We find that the number of simulations needed to sufficiently approximate our model with the DMD approximation is roughly $1,000$ (Figure S4).
...
Even when the estimation error variance is $0.1$, both the fixation probability and fixation time DMD approximations are accurate~(Figure S5). 
...
We found again that results of the DMD approximation are similar to those from stochastic simulations of the full model~(Figure S6).
...
\begin{figure}[h]
    \includegraphics[width=0.9\linewidth]{../figures/kimura_var.pdf}
...
\begin{figure}[H]
	\centering
    \includegraphics[width=\linewidth]{../figures/ch_env.pdf}
...
See Figure S7 for the scenario where $k>l$.
Here, population size is $N=1,000$;
In panel A, $A=0.9$, $\hat{A}=1$ ($1-\beta=s=0.005$), $k=20$, and $l=80$;
...
\begin{figure}[h]
    \includegraphics[width=\linewidth]{../figures/choose_bias.pdf}
  \caption{
...
Here, the x-axis is the index of the choosing copier, population size $N=200$; estimation error is normally distributed $e \sim N(0,\eta^2)$ with standard deviation $\eta=$0.0001 (blue), 0.001 (orange), 0.01 (green), markers are average of $300$ simulations.
}	
...
When $k>l$, strong success (high $\alpha$) will increase the fixation probability (Figure S7),
