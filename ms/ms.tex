...
%TODO check this - is influence frequency dependent?
...
%TODO If you omit the indirect trait, it is no longer indirect transmission.
...
In both we assume that the bias weight is either completely homogeneous, $\alpha_{i,j}=\alpha$, or that $\alpha_j$ is a bias of role-model $j$ only, meaning it does not vary between copiers.% TODO but result 1 has alpha_i
...
The proof of \Cref{cor:GBD} is in Appendix~\ref{sec:GBD}.
\\
...
% TODO where is the proof for the second part of the corollary??
\end{proof}
...
To validate that our approximation is correct, % TODO which approximation?? be explicit. refer to an equation. tell us what you are approximating.
...
When estimation error is applied, we sample $J_i$ for each copier $i$ from a normal distribution with expected value zero and variance $\eta^2$. % TODO J_i? e_i?
...
Then,
\begin{equation} 
% TODO the use of big O is incorrect... 
...
\begin{figure}[h]
  ...
  Markers are averages of $10,000$ simulations, error bars show 95\% confidence intervals for panels A and B and 75\% for panels C and D.
   ...
   $A/\hat{A}$, is between 0.01 and 0.99 (panels C and D); % TODO not clear, because x label suggest you modify beta, but caption says you modify A/hatA
...
\end{equation}
where $S_n=\sum\limits_{t=1}^{n} N (1-\beta_t)$ and $\beta_t$ is $\beta(A)$ at generation $t$. % TODO define more explicitly - connect it to the paragraph above that explains that the environment changes
\end{result}
