...
Influence bias also differs from {conformity}, which depend on the frequency of a trait in the population or in a sample of role-models, rather than the social dynamics of copying.
...
We follow the {Boyd and Richerson model} \citep{evolutionBook}, assuming only oblique transmission of the trait and omitting the indirect trait in order to reduce model complexity. 
{Consider} a population of $N$ individuals, described by a single trait {that takes continuous values}.
Each generation, $N$ naive individuals, or copiers, choose a single individual from the entire previous generation, or role-models, from which they will copy their trait.
This is different than the usual models studied by Feldman et al \citep{cumul_culture,anticonformity} or Boyd \& Richerson \citep{evolutionBook}, where the population is infinite and each copier samples $n$ role-models, and copies the trait from them. 
...
In both we assume that the bias weight is either completely homogeneous, $\alpha_{i,j}=\alpha$, or that $\alpha_j$ is a bias of role-model $j$ only, meaning it does not vary between copiers.
...
To validate that our approximation is correct
...
Frigyik et al.~\citep[section 2]{dirichlet} prove that the proportion of different colored balls in a \emph{P\'{o}lya urn model} converges to the Dirichlet distribution as the number of draws approaches infinity, $N$ in our model,
...
\paragraph{Numerical validation.}
To validate our model can be approximated by the Dirichlet distribution properly, i.e  (\Cref{cor:dirichlet}),
and test its sensitivity to the assumptions ($e_i=e$ and $\alpha_i=\alpha$ for $i=1,\ldots,N$) we compare it to results of stochastic simulations of the model, meaning we compare \Cref{cor:dirichlet} to  \Cref{eq:binary-model}.
...
When estimation error is applied, we sample $J_i$ for each copier $i$ from a normal distribution.
...
Then,
\begin{equation} 
E[x'-x] = \mathcal{O} (x(1-x)(1-\beta)) \;, 
\quad
V(x'-x) = \mathcal{O} (x(1-x)\Big(\frac{1}{\alpha N + (1-\alpha)}\Big)) \;.
\end{equation} 
\end{result}
...
\begin{figure}[h]
  ...
  Markers are averages of $10,000$ simulations, error bars show 95\% confidence intervals for panels A and B and 75\% for panels C and D.
   ...
   $A = a \cdot \hat{A}$ , where $a$ is a modifier such that $0.01 \le a \le 0.99$ (panels C and D);
...
\paragraph{Numerical validation.}
To validate that \Cref{eq:ch_env} can approximate our dichotomous model, we compared the results of this equation against simulations using \Cref{cor:dirichlet}. We find that the approximation fits the simulation results well for variable bias weights, $\alpha$, which corresponds to the effective population size (\Cref{fig:ch_env_alpha_beta}A).
...
%\end{split}
\end{equation}
Now because $x$, the initial frequency, is a constant, $V(x) = 0$,
and therefore
...
\begin{figure}[h]
  ...
  We performed computational simulations of the role-model choice process, i.e the fully iterative process, and compared them to the results of simulations using the DM distribution approximation.
...
\begin{figure}
  ...
  Here, population size, $N=1000$; success bias weight normally distributed, $\alpha\sim N(0.5,x^2)$ where $e^{-7}\le x \le e^{-1}$; 
