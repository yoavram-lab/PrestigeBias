...
\begin{figure}[h]
  ...
  \caption{\textbf{Fixation probability and time in a constant environment.}
	...
	Panel B has three lines: solid for  also shows our approximation for $\alpha=0.01$, and dashed for $\alpha=0$ (bottom) and $\alpha=0.02$ (top).
	Markers are averages of $10,000$ simulations, error bars show 95\% confidence intervals for panels A and B and 75\% for panels C and D.
   	Here, population size is $N=1,000$; In panels A and B, $A=0.7$, $\hat{A}=1$, and $1-\beta=0.044$; In panels C and D, $\alpha=0.01$, $0.01 < A< 0.99$, and $\hat{A}=1$, which determines $1-\beta$ via $\beta = \beta(A)/\beta(\hat{A})$ and \cref{eq:success_bias}.
   }
  \label{fig:var_alpha}
\end{figure}
