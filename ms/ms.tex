...
First, the the fixation probability increases (\Cref{fig:var_alpha}B) and the fixation time decreases (\Cref{fig:var_alpha}D) as a function of the success coefficient $1-\beta$, which acts as an effective selection coefficient.
Second, the fixation probability increases with the success-bias weight $\alpha$ ({fig:var_alpha}A), reaching a maximum at $2(1-\beta)=2s$ when there is no prestige bias ($\alpha=1$).
However, conditional on fixation, the fixation time is actually \emph{shorter} with low values of $\alpha$, that is, when prestige bias is strong. This is because prestige bias accelerates the evolutionary dynamics due to a \emph{rich-get-richer} process  (\Cref{fig:var_alpha}D)
...
Otherwise, fixation probability increases with the success coefficient ($1-\beta$, \Cref{fig:ch_env_alpha_beta}B) and becomes larger as $k-l$ increases, i.e., as the number of generations in which the invading phenotype is favored increases (\Cref{fig:ch_env_alpha_beta}C).
