...
Here, we study the joint effect of success and prestige bias on cultural evolutionary dynamics using mathematical analysis and stochastic simulations.
...
% TODO this is wierd
From \Cref{cor:GBD} we see that without success bias ($\alpha=0$), the expected number of copiers of each role-model is one. Hence, prestige bias is not frequency dependent.
...
Similarly, when the success coefficient $1-\beta$ is large, the approximation can break, as the fixation time can be lower then the number of generations in the cycle, $n$ (see \Cref{fig:var_alpha}D), and again the average selection coefficient, $S_n/n$ is not a good estimate of the effective selection coefficient.
...
On increasing the success coefficient $1-\beta$ to more than 0.15, the simulation results were located above the changing environment approximation, and below the constant environment approximation (\Cref{fig:ch_env_alpha_beta}B). This is probably because when the success coefficient is high, the number of generations may be less than $n$, the number of generations in a full cycle, and thus $S_n/n$ is not a good estimator for the `average selection coefficient'.
With even higher success coefficient values, the fixation probability exceeds even the constant environment approximation, possibly because the diffusion-equation approximation assumes weak selection, or in our case, weak success bias.
...
Next, we examined what happens in the continuous-trait model if the $i$-th copier evaluates its own optimal success-bias weight, $\alpha^*_i$, which minimizes the expected squared error between the chosen trait value and the `successful' trait value $\hat{A}$,
