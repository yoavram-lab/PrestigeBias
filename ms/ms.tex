...
\citep[Ch.~8]{evolutionBook}.
Prestige bias may be more common in humans than success bias \citep{complexityPaper}, and prestige is often mentioned in the cultural evolution literature, although there are few models of it; see Chellappoo (2021) for a philosophical critique of the concept of prestige itself. % TODO convert to reference; add this sentence to response letter.
...
Jimenez \& Mesoudi~\citep{prestigeSocialLearn} suggest that cultural evolution theory is a major factor in human ecological success. They mention that a way to acquire adaptive social information is by preferentially copying competent individuals within a valuable domain, and they call it success bias as we do in this paper. They also mention that competence within a domain is often difficult or impossible to directly asses, therefore people tend to use indirect cues of success, as suggested by Henrich and Gil-White~\citep{prestige_evolution}.

Henrich and Gil-White~\citep{prestige_evolution} noted that ``the most skilled/knowledgeable role-models will, on average, end up with the biggest and most lavish clienteles, so the size and lavishness of a given model's clientele size (the \emph{prestige}) provides a convenient and reliable proxy for that person's information quality''. Therefore, they predicted that skilled individuals have higher status, people preferentially copy them more than others, and therefore prestigious individuals may be influential even beyond their domain of expertise. 

Here, we examine the effect of both success and prestige biases, and their relationship in contribution to the cultural evolution of populations. We call the joint contribution of both biases \emph{influence bias}.
...
Ze Hong~\citep{payoff}, for example, presented a model that combines both frequency-dependent bias (including conformist bias) and payoff bias. He has shown that an integrated learning strategy may be superior to relying on either bias alone. Moreover, both are likely to contribute to the spread of variants with high payoff, and the magnitude of conformist bias affects the evolutionary dynamics.
% TODO add Denton; others
