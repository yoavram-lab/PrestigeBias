...
\begin{titlepage}
	\centering
	\includegraphics[width=0.4\textwidth]{ReichmanLogo.png}\par\vspace{1cm}
...
We combine success and influence bias together to form a ``prestige bias'', and analyze its effects on cultural-evolutionary dynamics using mathematical analysis and stochastic simulations.
We find analytic approximations to the stochastic role-model choice process, which facilitate further mathematical analysis and reduces the computational complexity of simulations. We validate these approximations using simulations, and demonstrate their robustness to model assumptions.
...
Our model extends the literature that attempt to describe of cultural transmission, 
especially in human societies where social media is popular and thus influence bias may be expected. 
Further work is needed to test if this model could predict various phenomena in cultural evolution.
...
Although cultural transmission is most common in humans, it is also observed in other animals, e.g., chimpanzees \citep{chimpsPrestige, chimpsCopy}, dolphins and whales \citep{dolphins_whales}, elephants \citep{elepahntsRepo}, and even flies \citep{fliesPaper}.

Cultural transmission may combine vertical transmission, where parents transmit to their offspring; oblique transmission, where adults (teachers, leaders, strangers) transmit traits to unrelated offspring; and horizontal transmission, where peers from the same age cohort transmit to one another. 
...
It has been demonstrated that non-vertical cultural transmission can maintain maladaptive traits, which can be beneficial in changing environments \citep{Ram2018}. 

Vertical transmission is also possible in the opposite direction: parents may copy traits from their offspring \citep{transmissionVectorsBook,transmissionVectors}.


Cultural transmission biases may cause a trait to have a disproportionate probability to be transmitted compared to its frequency in the population.
...
% TODO add definition of success bias and some reference to a study demonstrating it in humans or other animals.
Indeed, in a tournament between learning strategies conducted by \citet{strategiesPaper}, most winner included a mixture of success-biased social learning and individual learning, implying that success-biased learning is a good strategy, but that by itself is not enough, even if success is measured accurately.

\citet[Ch. 5]{evolutionBook} have suggested that the evaluation of success can be divided into three groups: \textit{direct bias}, \textit{indirect bias} and \textit{frequency-dependent bias}.
A direct bias occurs when one phenotype is more attractive than other phenotypes, and is evaluated by \textit{directly} testing the trait.
...
For example, when the common paddle grip is used by 60\% of the demonstrators, then if the probability to copy this grip is 80\%, then transmission is under positive frequency bias, also called conformity; if the probability is 40\%, then transmission is under negative frequency bias, or non-conformity \citep{conformism}. 
The effects of conformity and non-conformity on cultural evolution have been studied with both models \citep{conformity,anticonformity} and experiments \citep{negativeFrequency}.
...
\citet[Ch.~8]{evolutionBook} and \citet{complexityPaper} suggest that prestige biases are probably more common in humans than success biases.
...
Influence can be easily estimated, so costs may be lower than estimating success. In some cases it could be a more accurate estimation as well.

Influence bias is a context-based, rather than a content-based bias--it does not depend on the phenotype but rather on the number of copiers that already copied each role-model. IT also differs from frequency biases such as conformity \citep{conformity}, which depend on the frequency of a trait in the population or in a sample of role-models rather than the social dynamics of copying.

Here, we define a model of cultural transmission with prestige bias that combines both success and influence biases, find analytic approximations for this model, and analyze its dynamics. We find approximations for the probability and time for fixation of a "successful" phenotype (i.e., that is favored by success bias). Comparing these approximations to Kimura's approximations for the fixation of a favorable allele \citep{kimura,kimura_average}, we demonstrate that success and influence bias play the role of natural selection and genetic drift, respectively. 
...
$\E[K_{N,j}] = N \cdot G_{1,j}$ if trait estimation error is uniform for all copiers ($e=e_i$ for $i=1, \ldots, N$). This means that there's no actual estimation error in the population, just a fixed bias in every copier estimation.
...
When $k$ is the number of generations where the invading phenotype is favored, and $l$ when the original trait is, we found that for large $k$-to-$l$ ratio (while keeping constant total cycle length, $n=k+l=100$), the changing environment approximation, \cref{eq:ch_env} converges to the constant environment approximation, \cref{eq:kimura_p}, see \Cref{fig:ch_env_alpha_beta}C and \Cref{fig:ch_env_alpha_beta}D.
...
\end{result}
The proof is seen in \Cref{fig:ch_env_alpha_beta}A,B. The approximation follows the trend of the simulation results for all $\alpha$ values.
