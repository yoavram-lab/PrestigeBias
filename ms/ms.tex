...
Influence bias also differs from {conformity}, which depend on the frequency of a trait in the population or in a sample of role-models, rather than the social dynamics of copying. % TODO check this - is influence frequency dependent?
...
We follow the {Boyd and Richerson model} \citep{evolutionBook}, assuming only oblique transmission of the trait and omitting the indirect trait in order to reduce model complexity. % TODO If you omit the indirect trait, it is no longer indirect transmission.
{Consider} a population of $N$ individuals, described by a single trait {that takes continuous values}.
Each generation, $N$ naive individuals, or copiers, choose an individual from the previous generation, or role-models, from which they will copy their trait.
% We need to mention that each copier chooses one role model out of the entire population; this is different from other models studied by Feldman and colleagues in which the population is infinite and each copier samples n role models, and uses those n role models to determine its trait:
...
% Denton, Kaleda Krebs, Yoav Ram, Uri Liberman, and Marcus W. Feldman. 2020. “Cultural Evolution of Conformity and Anticonformity.” Proceedings of the National Academy of Sciences 117 (24): 13603–14. https://doi.org/10.1073/pnas.2004102117.
% Boyd & Richerson book
...
Here, the bias weight $\alpha_{ij}$ determines the relative weighting of success and influence bias. It is a characteristic of the interaction of role-model~$j$ with copier $i$ that determines the relative significance of direct success vs. influence in the role-model's overall prestige in the eyes of the copier.

{Different individuals} may evaluate the importance of success and influence differently. Additionally, we assume {each} role-model displays {its} influence and success individually. For example, {individuals} with more followers but {lacking} skill may emphasize the number of {their} followers rather than {their} skill. %We therefore defined $\alpha_{i,j}$ to be the relation between every copier's intrinsic significance of estimation, and every role-model's personal emphasis.
...
In both we assume that the bias weight is completely homogeneous, $\alpha_{i,j}=\alpha$ (it is also enough to assume that $\alpha_j$ is a bias of role-model $j$ only, that is, it does not vary between copiers). % TODO but result 1 has alpha_i
...
\end{proof}
This result gives the following two corollaries on the expected number of followers of a given role-model $j$ by the end of the role-model choice process, $K_{N,j}$. The proof of \Cref{cor:GBD} is in Appendix~\ref{sec:GBD}.
\\
...
\begin{proof}
{Drezner and Farnum} \citep[eq.~(2.3)]{GBD} show that 
$\E[k] = N \cdot Q_j(1,1)$ (modified from their notation). % TODO what is E[k]?
...
\begin{corollary}\label{cor:GBD} % TODO merge with the previous corollary
...
To validate our results  % TODO be more explicit - what exactly are you validating?
...
We used the average of the $1,000$ simulations to eliminate drift errors, as can be seen in \Cref{fig:GBD}. % TODO what are drift errors? not clear.
...
Frigyik et al.~\citep[section 2]{dirichlet} prove that the proportion of different colored balls in a \emph{P\'{o}lya urn model} converges to the Dirichlet distribution as the number of draws approaches infinity, % TODO is the "number of draws" equal to our parameter N?  if so, did we check the sensitivity of this approximation to small N?
based on the \emph{Martingale Convergence Theorem} \citep{martingaleBook}.
...
\paragraph{Numerical validation.}
To validate our analytical result (\Cref{cor:dirichlet}) % TODO again be more explicit what it is you are validating
and test its sensitivity to the assumptions ($e_i=e$ and $\alpha_i=\alpha$ for $i=1,\ldots,N$) we compare it to results of stochastic simulations of the full model. % Also not clear what is the "full model" - ref to equation, and explain - you are comparing the DM approximation (result 2? corollary 3? eq 16? what?) to results of simulations of the model (not full), eq 10?
First, we compute an observed distribution of the number of copiers from the average empirical distribution of multiple simulations.
We then {compare} this observed distribution with the expected theoretical DM distribution (\Cref{fig:DM_validation}A). The difference in distributions was not perceived when plotting both distributions on the same figure, so we used the difference instead. The maximum difference is 0.5 role-models, which {indicates} a very good fit.
...
We examined the robustness of the DM approximation to relaxing the approximation assumptions. % TODO which assumptions? those given in result 2? if so, refer to them directly and repeat them (e=ej, alpha=alpha_j).
...
When estimation error is applied, we sample $J_i$ for each copier $i$ from a normal distribution with varying scale (variance). % TODO Not clear what “varying scale (variance)” means
Even when the standard deviation is $0.1$, the fixation probability and time {are} similar~(\Cref{fig:hetro_error}). 
...
% TODO check that is its clear we are using the dichotomous model here
...
% TODO I'm not sure this needs to be a "Results" - I think we can write it here as apart of the analysis, which gives Result 4. Also, where is the actual proof of the equation in this result?
...
% TODO can we say beta=beta(A)/beta(hat A) ? then we dont force beta(hat A)=1
Then,
\begin{equation} % TODO remove \approx and instead add big-O notation
...
\begin{figure}[h]
  ...
  Markers are averages of $10,000$ simulations, error bars show 95\% confidence intervals for panels A and B and 75\% for panels C and D.
   Here, Population size, $N=1,000$; phenotype values, $\hat{A}=1$, $A=0.7$ (panels A and B), 
   $A = a \cdot \hat{A}$ with $0.01 \le a \le 0.99$ (panels C and D); % TODO this is not clear
...
\end{equation}
where $S_n=\sum\limits_{t=1}^{n} N (1-\beta_t)$. % TODO beta_t is note defined - maybe in the appendix? needs. to be defined above, before the reference to the appendix
\end{result}
...
%\end{split}
\end{equation}
Now because $x$ is a constant, $V(x) = 0$, % TODO x is not a constant, go back to Durrett and check how to go from the previous equation to the next one
and therefore
...
\begin{figure}[h]
  ...
  The approximation (orange) fits simulation results (blue) well when using 1,000 simulations for both models. % TODO which two models?
...
\begin{figure}[h]
  ...
  We performed computational simulations of the role-model choice process and compared them to the DM distribution. % TODO it is not clear what's being compared here
...
\begin{figure}
  ...
  Here, population size, $N=1000$; success bias weight normally distributed, $\alpha\sim N(0.5,x^2)$; % TODO what is x here?
