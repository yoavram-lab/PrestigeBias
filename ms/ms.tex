...
Indirect bias occurs when an individual uses a phenotype to evaluate the attractiveness of another individual. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section*{Models}
We begin with a continuous trait model with indirect bias suggested by Boyd and Richerson~\citep{evolutionBook}, propose an extension with influence bias, and then develop a model with a dichotomous trait. The indirect bias is due to an indirect evaluation, using a certain trait to determine success of others.
...
$\E[k] = N \cdot Q_j(1,1)$, where $k$ is the number of copiers that chose role model $j$ (i.e successful trials).
...
From \Cref{cor:GBD} we see that if there is no success bias, meaning $Q_j(1,1)=q$ for $j$ in $1,...,N$, then the expected number of copiers will be Binomial distributed. This proves that influence bias is not frequency dependent.
...
We did our validation on a relatively small population size $N=100$, showing that the approximation is very good for small $N$ as well, even though the Martingales proof requires $N$ to be very large.
...
Let $x$ and $x'$ be the frequency of type $\hat{A}$ in a population with $N$ individuals in the current and next generation, and  $\beta$ be the success coefficient of phenotype $A$, $\beta = \frac{\beta(A)}{\beta(\hat{A})} < 1$.
...
\begin{figure}[h]
  ...
  Markers are averages of $10,000$ simulations, error bars show 95\% confidence intervals for panels A and B and 75\% for panels C and D.
   ...
   $A/\hat{A}$ varies between 0.01 and 0.99 (panels C and D), and $\hat{A}=1$, which affects $\beta$ given in \Cref{eq:drift_diff};
...
Following \citep{changeEnv}, we denote $k$ as the number of generations in which the invading phenotype is favored by success bias, and $l$ as the number of generations in which the resident phenotype is favored by success bias. This means that during the $k$ generations of the cycle, $\beta=\frac{\beta(A)}{\beta(\hat{A})}$, where $\hat{A}$ is the invading phenotype. During the following $l$ generations of the cycle, the favored phenotype is switched, so that $\frac{\beta(A)}{\beta(\hat{A})}>1$.
...
\end{equation}
where $S_n=\sum\limits_{t=1}^{n} N (1-\beta_t)$ and $\beta_t$ is $\beta(A)$ at generation $t$. % define more explicitly how environmental change affects A and beta. define it before the result! (above)
...
\begin{result}[Fixation probability in changing environment]\label{res:ch_expected}
...
\begin{figure}[h]
  ...
  We performed computational simulations of the role-model choice process (\Cref{eq:prestige}) and compared them to simulations when using the DM distribution approximation (\Cref{cor:dirichlet}).
...
\begin{figure}
   ...
   Both the DM approximation (orange) and Kimura's equation (black line) fit the stochastic simulations (blue) well even with a high variance in success bias weight.
   Markers for average across $5,000$ simulations, error bars are 95\% confidence intervals.
  Here, population size, $N=1000$; success bias weight is not homogeneous, but instead normally distributed, $\alpha \sim N(0.5,\epsilon^2)$ where $e^{-7}\le \epsilon \le e^{-1}$; 
