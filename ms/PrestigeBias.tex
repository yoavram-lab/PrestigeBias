...
For high $\alpha$ values (above 0.5), very few simulations were needed to reach p-value of 1 (less than five simulations for $\alpha=0.9$, and less than 20 for $\alpha=0.7$).
For very low $\alpha$ values, which means very high influence weight, the number of simulations needed was 100.
When testing our observed distributions against different distributions, for example the uniform distribution (all role-models have exactly one copier), the p-value was 0, so that the null hypothesis was rejected, as required. % TODO did you test other distributions?
...
%In the dichotomous model the estimation error will be added by using heterogeneous $J$ parameter, which controls the strength of the bias of \cref{eq:success_bias}. % TODO do we need this sentence?
...
We also relaxed our assumption of a uniform bias weight $\alpha$ (i.e., $\alpha_i=\alpha$). We allowed $\alpha$ to vary in the population, drawing $\alpha_j$ for each role-model $j$ from a normal distribution such that $\alpha_j \sim N(X,X)$. % TODO replace X and X with relevant symbols
...
\begin{figure}[h]
  ...
  The approximation (orange) fits simulation results (blue) well when using 1,000 simulations. % 1000 simulations of the model or of DM ??
  ...
  % TODO
  % remove legend
  % change y labels: "Fixation probability" "Fixation time"
  % change x label: "Number of simulations"
...
\begin{figure}
  ...
  \caption{\textbf{Robustness of DM approximations to variation in the bias weight $\alpha$.} 
   High success weight variance distances the approximation and the full model of generations to fixation from the Kimura's approximation, but not by much (confidence intervals still cover it). Error bars are 95\% confidence intervals, and are less condensed (+- 0.03 probability and +-40 generations)
...
We focus on the fixation probability and fixation time of an advantageous phenotype, similar to analyses in population-genetic models. % References: 
...
\begin{figure}[h]
  \begin{center}
    %TODO remove the (a) and (b) from the latex and put them on the figure itself (top left corenr).
  \begin{subfigure}[a]{0.49\linewidth}
   ...
   \end{subfigure}
  \end{center}
  \caption{Comparison of the DM approximation and the WF model for different values of the effective population size.
  Error bars show mean and 95\% confidence intervals.
  Effective population, $N_e=\alpha N + (1-\alpha)$.
  $5,000$ simulations per data point; Population size, $N=1,000$; $\hat{A}=1$, $A=0.7$,$J=1$,$1-\beta=s=0.044$.}
  \label{fig:var_alpha}
  % TODO
  % - better x and y label, as noted above
  % - in legend, replace "kimura" with "Approx."
  % - in legend, remove "=2s"
  % - change approximation color, it is too similar to WF simulations
  % - make the horizontal line black?
  % - caption should start with bold sentence summarizing the figure. 
  % - in caption add description of each parameter (as I did for N)
  %
\end{figure}
...
\begin{figure}[h]
  ...
  % TODO 
  % - see previous figure
  % - consider making the error bars smaller with more simulations or narrower CI (50%?)
  % - consider using plt.xscale('log')
  % - maybe you can merge this figure with the figure?
  \caption{Comparison of the DM approximation and the WF model for different values of the selection coefficient, manifested as success bias in our model. Error bars are 95\% confidence intervals.
...
\paragraph{Changing environment.}
...
\begin{figure}[h]
  ...
  $10,000$ simulations per data point, $N=1,000$, $\hat{A}=1$, $A=0.9$, $J=1$, \textbf{(a)} success bias/selection coefficient is: $1-\beta=s=0.005$, \textbf{(b)} success weight is: $\alpha=0.1$. 
  }
  % TODO 
  % see previous figures..
  % not sure you need the green curve in the left panel
  % the right panel might be better with yscale('log')? or xscale('log')? right now it is hard to see that the blue even changes...
  % 
...
However, $\beta$ does. 
We plotted along the modified estimation the original Kimura's estimation, as a limiter. % TODO rephrase, this sentence is not clear at all
We suspect that when $\beta$ is too large, there won't be many cycles in the simulations. This might happen if either the population reaches a high frequency of the fitter phenotype after only a few cycles, or it get extinct very quickly, % TODO what gets extinct? what does "it" refers to?
 because the advantage it had in the $k$ generations wasn't sufficient, and the same $s$ becomes a greater disadvantage when the environment changes, resulting in fast extinction. % TODO read and rephrase.
For lower % TODO this was "higher" but should be "lower" right?
values of $\beta$ we see a deviation from the original estimation environment, % TODO what is "estimation environment"?
but it's to be expected, because Kimura's equations weak fitness (i.e., low selection coefficients, $s$).
...
\begin{figure}[t]
  ...
  \caption{}
   ...
   \end{subfigure}
  \end{center}
  %TODO add panel labels (a) and (b) on figures corner.
  % see previous figures todos
  % move legend to upper left? it is currently hiding something
  \caption{Model simulations compared with both the constant environment and the changing environment equations for different compositions of the environment cycle. % TODO reminder what k and l are.
   When $k<l$ the approximation is good. When $k>l$, the approximation and the simulations are both very close to the constant environment approximation. 
...
We found that larger $k$-to-$l$ ratio increases the agreement between the modified equation and the original estimation of the constant environment. % "modified....environment" unclear.
...
When $n$ is high, there will be less cycles, % TODO why higher n gives less cycles?
and the simulations will get closer to the constant environment equation.
...
However, it has been suggested \citep{complexityPaper} that because estimating success is hard, a more common bias is \textit{prestige bias}---a bias towards role-models perceived to be successful by some other indicator.
...
Leaders may lead the group when traveling, decide on a nesting site, or choose foraging patches. They found that leadership is mostly observed when there is a profound social network in the group, % TODO profound sounds weird here. what do you mean by it?
and when there are individuals that present leadership behavior.
...
When no individual posseses such traits, or when the social network is lacking, % TODO lacking what?
decisions are more likely to be made by consensus.
Leader may carry selection costs, e.g. due to higher predation risk, poisoning when exploring new foraging patches.
In some cases, %e.g. fish, % TODO ref for fish? otherwise don't mention fish
 the leader may be the hungriest or the weakest individual, while the others would prefer to follow the leader and minimizing their risks and costs.
...
They show that leaders wouldn't be chosen due to their superior abilities, but because they have the least to lose. % TODO are you sure? this is in baboons?
...
In modern society it is less common to worry about mere survival, and so the prestigious positions, even though they are not without risks and costs, are not as dangerous as for animals in nature. % TODO reference? I'm not sure this is correct
This may be the reason humans strive for the prestigious positions, as they may reap rewards greater than the risk and costs to achieve them. % you give two reasons - "this may be the reason" and "as they may reap..." I think the latter is more correct. also I think it is personality and family driven, to an extent.
This is in complete contrast to animals, where the weakest/hungriest is driven to lead, compared to humans where leadership positions are mostly competitive. % TODO I'm not sure, let's discuss!
...
and (outstanding) personal qualities. This description is very similar to success bias in our model. % TODO I changed the end of the sentence, is it ok?
...
Our model does not reflect these leadership styles, but several adjustments could be made in order to match it, such as assuming there is a correlation between phenotype to leadership style. This would require assuming some costs and benefits so that prestige has higher  rewards but greater costs, while dominance has lower rewards and lower costs. % TODO why?
...
Our model can be expanded in various ways: observing the effects of different bias functions, % TODO unclear
