...
\begin{figure}[h]
  ...
  $10,000$ simulations per data point, $N=1,000$, $\hat{A}=1$, $A=0.9$, $J=1$, \textbf{(a)} success bias/selection coefficient is: $1-\beta=s=0.005$, \textbf{(b)} bias weight is: $\alpha=0.1$. 
  }
   %TODO 
  % see previous figures..
  % not sure you need the green curve in the left panel
  % ANS - to show it's not a valid option to use?
...
\begin{figure}[h]
  ...
  \caption{Model simulations compared with both the constant environment and the changing environment equations for different values of $k,l$, where $k$ is the number of generations where the bias is advantageous, and $l$ are the generations it is disadvantageous.
 ...
 We ran $10,000$ simulations per data point, $N=1,000$, $\hat{A}=1$, $A=0.8$, $J=1$, $1-\beta=s=0.02$, $\alpha=0.1$.}
