...
We are mainly interested in the effect of the bias weight, $\alpha$, which determines the relative effect of success and influence on prestige bias. % TODO ref eq 10?
For simplicity, we do not include role-model estimation error in this analysis. % TODO meaning epsilon=0?
...
Similarly, we can use $1-\beta$ and $N_e$ in the population-genetics fixation time approximation given by \citet[eq.~8]{kimura_average}.\\
...
\end{equation}
% TODO double check this, because eq 17 in Kimura 1969 looks different.
% in particular, I don't see Ne here...
\end{result}
...
\begin{figure}[h]
% TODO 
% capital letters in panel labels
% bigger font
    \includegraphics[width=\linewidth]{../figures/binary/kimura_var_alpha.pdf}
  \caption{\textbf{Fixation probability and time approximation for varying bias weight.}
  ...
  Fixation probability (a) is limited at approximately $2s$ (blue), as in the classic WF model.
  Markers are averages of $5,000$ simulations, error bars show 95\% confidence intervals.
...
\begin{figure}[h]
  ...
  \caption{\textbf{Fixation probability and time approximation for varying success coefficient.}
  % TODO
  % merge with previous figure; see comments there.
   The approximation (black) fits DM simulations results (green) and Wright Fisher simulation results (orange) when using \cref{eq:kimura_p}.
...
\quad
V(X_t/N ) \simeq x(1-x) t/N_e \;, % TODO shouldn't it be V[X_t/N - x] ?? or something like that? with square brackets and with "- something" ?
\end{equation}
...
% TODO check this - looks weird that you have such a similar expression on the left side
\begin{split}
...
% TODO what are you saying in the next paragraph?
...
For the very low $\beta$ values (0.65 and below),% TODO is that really "very low?"
the fixation probability exceeds even the constant environment approximation (which is the upper limit). However, this is to be expected, because the diffusion-equation approximations assume weak selection (i.e., low selection coefficient $s$).

% TODO what is k and l?
We found that for large $k$-to-$l$ ratio (while keeping constant total cycle length, $n=k+l=40$) the changing environment approximation (\cref{eq:ch_env}) converges to the constant environment approximation (\cref{eq:p_kimura}), see \Cref{fig:ch_env_k_l}. 
When using higher values of $n$, the agreement between the approximation and simulation results is weaker. % TODO reference to figure?
This is because our approximation is more precise when more cycles occur in the simulations. % TODO "in the simulations"-right?
When $n$ is high, the cycles are longer, the dynamics are therefore similar to a constant environment, and fixation or extinction is more likely to happen before the next cycle. Thus, there will be less cycles, which lowers our approximation precision.


\begin{figure}[h]
% TODO
% See comments on previous figures
% make x axes as before, with alpha on bottom and Ne on top
    \includegraphics[width=\linewidth]{../figures/changed_env/ch_env_var.pdf}
  \caption{\textbf{Sensitivity of fixation probability approximation in a changing environment to varying success coefficient and bias weight.}
...
\textbf{(b)} When success bias is large ($\beta > 0.1$),  % TODO it was s>0.1, and O changed to beta>0.1 because that's what the x axes says, but I am not sure, as below you say the approximation works for weak selection, and s=1-beta, so large beta should give weak selection... please check
simulation results (orange) are underestimated by the approximation (blue; \cref{eq:ch_env}). With even larger success bias ($\beta > 0.35$), even the constant environment approximation (green; \cref{eq:kimura_p}) underestimate simulation results, likely because the diffusion equation approximation assumes weak selection.
Markers show average of $10,000$ simulations, error bars show are 75\% (a) and 95\% (b) confidence intervals.
...
\begin{figure}[h]
% TODO
% merge with previous figure and edit the caption accordingly.
