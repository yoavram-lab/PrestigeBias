...
%%% 
% instead, write here how the model parameters affect fixation probability
...
% TODO this is a nice paragraph, but it is unclear how it relates to your work. does your model results have something to say about leadership? or what? if this is just an introduction so that you can then present Van Vugt and Smith, then it is too long.
...
However, wrong decision making that harms the group could result in harm. % TODO you have double harm here, rephrase it.
In modern society, many humans strive for prestigious positions, as they may reap rewards greater than the risk and costs to achieve them, or due to individual personality and pressure or education from the family. % again, how is this relevant? also I would add reference to at least some of your claims -

\citet{dual_leadership} suggest a different view of leadership. They note that most discussions assume there is one type of leadership, 
as seen above, and so they differ in their definitions. % TODO this sentence (on this line) in unclear
\citet{dual_leadership} suggest a way to solve said contradiction % TODO what contradiction? I dont understand
 by defining two types of leaderships: prestige-based and dominance-based.
...
In contrast, Machiavelli views leaders as rulers who exercise influence by imposing costs through (the threat of) punishment and violence. % TODO but this is different from our influence model right?
...
The emerging cultural-evolutionary dynamics and their dependence on the costs and benefits are intriguing. % TODO is this paragraph is a suggestion for followup work? if so, you could make it shorter - remove the first sentences about contradictions, and directly present Confucius and Machiavelli, and so on. Also this should be in a latter part of the Discussion under the name "Future work" or something like that.

% TODO it seems that the next paragraph is the one that really corresponds to ours, so it should be emphasized earlier, I think
...
\citet{prestige_evolution} show that prestige evolved via natural selection as an efficient process to extract reproductive benefit from the flow of socially transmitted information. % TODO how do they show this? math? simulations? experiments?
Simply put, prestige naturally evolves when social learning already exists to save costs of individual learning. % TODO Are you sure ?
It could be interesting in the future to expand our model using this idea: observing the copier trait of \textit{evaluation}, rather than only observing the evolution of the trait copied. % TODO unclear
\citet{prestige_evolution} suggest that the most skilled role-models will, on average, end up with most copiers. Their research, definitions, and results, is consistent with ours. % TODO how so?

Furthermore, according to \citet{no_replication}, the process of cultural evolution doesn't require replication of representations.% TODO what is "representation"?
...
% TODO maybe this and the following should be grouped under a heading like "empirical evidence of prestige bias"
...
Their definition of prestige is similar to our influence bias. They showed that the odds of 3-4 years-old children learning from an adult model to whom bystanders had previously preferentially attended for 10 seconds were more than twice those of their learning from a model whom bystanders ignored.
They also note that prestige effects are domain-sensitive: they found that prestigious models were attended more when demonstrating artifact-use, but not as much as when presenting food preferences. % TODO confusing phrasing, try again?
This lead \citet{prestige_cultural_learning} to suggest that when the trait is costly to learn individually, prestige will have a higher effect. % TODO higher effect? or will be used more often?
...
They found that ``Fijian villagers (ages 10 and up) are biased to learn from others perceived as more successful/knowledgeable, both within and across domains (prestige effects).'' % TODO it's better to rephrase in your words. (you can have one quote, but too is a bit much)
Their research thus suggests that copying from those perceived as successful, rather than actually are successful. % TODO this sentence is not finished?
They show that the social networks representing copier--role-model relationships are centralized, suggesting that ``this degree of centralization is consistent with the prediction that people substantially share notions about who is a good cultural model (network centrality), but that individuals' model selections are influenced by multiple factors.'' 
...
Prestige bias can also accelerate reversal of harmful traditions such as child marriage, open defecation, % TODO why "open defecation" ??
...
% TODO I think this whole commented thing should be removed...
%We also displayed similarities between this process and genetic transmission, while presenting differences between them, specifically selective biases such as influence and prestige. We also presented examples of good and bad usages of such biases. All of these are mainly presented as a parallel process to the natural selection process in regards to physical anatomy, or at least have an indirect effect on it. 
...
% TODO I think this commented out part can be remove: In this paper we described several models and processes, analyzed them, and discovered some interesting approximations and results. But even though, there's much more to research in the direction we took, which can help us better model, understand, and maybe even predict, human behavior, specifically in this era where social media is very accessible and informative.
...
% TODO add a 4-5 sentence "Conclusions" paragaph
